...
# ═══════════════════════════════════════════════════════════════════════
# PARSER 1: DNI (VERSIÓN v4.1 - OPTIMIZADA)
# ═══════════════════════════════════════════════════════════════════════

def parsear_dni(texto_ocr, tipo_dni="DNI_AZUL"):
    """
    Extrae campos de DNI Azul o Electrónico (VERSIÓN v4.1 OPTIMIZADA)
    """
    import re
    datos = {}
    ...
    if ap1_match:
        datos["apellido_paterno"] = ap1_match.group(1).strip()

    # Segundo Apellido (puede ser compuesto: "BENAVENTE DE RUIZ")
    ap2_match = re.search(
    ...
    if ap2_match:
        datos["apellido_materno"] = ap2_match.group(1).strip()

    # ═══════════════════════════════════════════════════════
    # PRENOMBRES (VERSIÓN MEJORADA)
    # ═══════════════════════════════════════════════════════

    # Patrón 1: Capturar solo la línea inmediata después de "Prenombres"
    nombres_match = re.search(
        r'Prenombres?[:\s]*\n?\s*([A-ZÑÁÉÍÓÚ]+(?:\s+[A-ZÑÁÉÍÓÚ]+)*?)(?=\n|\d{6,8}|Sexo)',
        texto_ocr,
        re.IGNORECASE
    )

    if nombres_match:
        nombres_raw = nombres_match.group(1).strip()
    else:
        # Patrón 2: Nombres pegados (sin espacios)
        nombres_pegados = re.search(
            r'Prenombres?[:\s]*\n?\s*([A-ZÑÁÉÍÓÚ]{10,})(?=\n|\d{6,8})',
            texto_ocr,
            re.IGNORECASE
        )
        if nombres_pegados:
            nombres_raw = nombres_pegados.group(1).strip()
        else:
            nombres_raw = None

    if nombres_raw:
        # Quitar apellidos si se colaron
        if "apellido_materno" in datos:
            for parte in datos["apellido_materno"].split():
                nombres_raw = nombres_raw.replace(parte, "").strip()

        # Si está pegado, separar nombres comunes
        if ' ' not in nombres_raw and len(nombres_raw) > 10:
            nombres_raw = re.sub(
                r'(MARIA|MONICA|MILAGROS|JUAN|JOSE|LUIS|CARLOS|PEDRO|ANA|ROSA|CARMEN|TERESA|ISABEL|GLORIA|PATRICIA|ALEJANDRA|FERNANDA|CAROLINA|DANIELA|ANDREA|CLAUDIA)',
                r'\1 ',
                nombres_raw
            ).strip()

        # Limpiar espacios múltiples
        nombres_raw = re.sub(r'\s+', ' ', nombres_raw)
    ...
    for pattern in fnac_patterns:
        match = re.search(pattern, texto_ocr, re.IGNORECASE)
        if match:
    ...
    nac_match = re.search(r'Nacionalidad[:\s]*([A-Z]{3})\b', texto_ocr, re.IGNORECASE)
    if nac_match:
    ...
    if dist_match:
        distrito = dist_match.group(1).strip()
        if "-" in distrito:
    ...
    emision_match = re.search(
        r'Emisi[oó]n[:\s]*(\d{2})[/\s]?(\d{2})[/\s]?(\d{4})',
        texto_ocr,
        re.IGNORECASE
    )
    if emision_match:
...
print("✅ Parser DNI v4.1 (nombres optimizados)")
