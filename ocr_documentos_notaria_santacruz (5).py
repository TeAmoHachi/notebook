...
"""
═══════════════════════════════════════════════════════════════════════
🔥 SISTEMA OCR INTELIGENTE PARA NOTARÍA SANTA CRUZ
═══════════════════════════════════════════════════════════════════════
...
✅ DNI Azul (formato antiguo)
✅ DNI Electrónico (formato nuevo)
...
try:
    ...
    print("💡 SOLUCIÓN:")
    print("   1. Ve a: Entorno de ejecución → Cambiar tipo de entorno")
    print("   2. Selecciona: Acelerador de hardware → T4 GPU")
...
print("📦 Instalando librerías con versiones compatibles...")
print("=" * 70)
...
# ═══════════════════════════════════════════════════════════════════════
# PASO 2: INSTALAR NUMPY CORRECTO (CRÍTICO)
# ═══════════════════════════════════════════════════════════════════════

print("\n🔧 Instalando NumPy 1.26.4 (compatible)...")
!pip install -q "numpy<2.0" --force-reinstall
print("   ✅ NumPy 1.26.4 instalado")

# ═══════════════════════════════════════════════════════════════════════
# PASO 3: INSTALAR PADDLEPADDLE 2.6.1 (VERSIÓN ESTABLE)
# ═══════════════════════════════════════════════════════════════════════

print("\n🔧 Instalando PaddlePaddle 2.6.1...")
!pip install -q paddlepaddle-gpu==2.6.1
print("   ✅ PaddlePaddle 2.6.1 instalado")

# ═══════════════════════════════════════════════════════════════════════
# PASO 4: INSTALAR PADDLEOCR 2.7.3 (COMPATIBLE CON 2.6.1)
# ═══════════════════════════════════════════════════════════════════════

print("\n🔧 Instalando PaddleOCR 2.7.3...")
!pip install -q paddleocr==2.7.3
print("   ✅ PaddleOCR 2.7.3 instalado")

# ═══════════════════════════════════════════════════════════════════════
# PASO 5: INSTALAR DEPENDENCIAS
# ═══════════════════════════════════════════════════════════════════════

print("\n🔧 Instalando OpenCV y dependencias...")
...
print("\n🔄 REINICIANDO KERNEL...")
print("   ⏱️ Se reiniciará en 3 segundos...")
print("\n💡 DESPUÉS DEL REINICIO (mensaje 'session crashed'):")
print("   1. Click en RECONNECT")
print("   2. Ejecuta celda 1 (verificar GPU)")
print("   3. Ejecuta celda 2B (verificación)")
...
print("📦 PASO 1: Instalando NumPy correcto...")
print("=" * 70)

# Limpiar versiones conflictivas
print("\n🧹 Limpiando NumPy anterior...")
...
import numpy as np
print(f"\n✅ NumPy instalado: {np.__version__}")

if not np.__version__.startswith('1.26'):
    print("   ⚠️ Versión incorrecta, reintentando...")
...
print("   1. Click en RECONNECT")
print("   2. ⚠️ NO ejecutes celda 2A de nuevo")
print("   3. ✅ Ejecuta celda 2B (instalar PaddleOCR)")
...
# ═══════════════════════════════════════════════════════════════════════
# CELDA 2B: INSTALAR PADDLEOCR (VERSIÓN MEJORADA - congela NumPy)
# ═══════════════════════════════════════════════════════════════════════

print("📦 PASO 2: Instalando PaddleOCR con NumPy congelado...")
print("=" * 70)
...
if not np.__version__.startswith('1.26'):
    ...
    print("   ✅ NumPy 1.26.4 correcto")

# ═══════════════════════════════════════════════════════════════════════
# Crear constraints.txt para BLOQUEAR NumPy
# ═══════════════════════════════════════════════════════════════════════

print("\n🔒 Creando restricciones para evitar upgrade de NumPy...")

with open('/tmp/constraints.txt', 'w') as f:
    f.write('numpy==1.26.4\n')
    f.write('scipy<1.14.0\n')  # Versión compatible con NumPy 1.x
...
print("   ✅ PaddlePaddle instalado")

# ═══════════════════════════════════════════════════════════════════════
# Instalar PaddleOCR (con restricciones de NumPy)
# ═══════════════════════════════════════════════════════════════════════

print("\n🔧 Instalando PaddleOCR 2.7.3 con restricciones...")
...
# ═══════════════════════════════════════════════════════════════════════
# Verificar que NumPy NO cambió
# ═══════════════════════════════════════════════════════════════════════

print("\n🔍 Verificando que NumPy no cambió...")
import importlib
importlib.reload(np)
import numpy as np
...
if not np.__version__.startswith('1.26'):
    print("   ❌ NumPy cambió a 2.x")
    print("   Forzando downgrade...")
    !pip install --force-reinstall -q "numpy==1.26.4"
    print("   ✅ NumPy restaurado")
else:
    print("   ✅ NumPy se mantuvo en 1.26.4")
...
print("=" * 70)
print("\n💡 Ahora ejecuta la celda 3 (Inicializar OCR)")

# ═══════════════════════════════════════════════════════════════════════
# CELDA 2C: ARREGLAR SCIPY (opcional, solo si celda 3 falla)
# ═══════════════════════════════════════════════════════════════════════

print("🔧 Arreglando scipy...")
...
try:
    ...
    print(f"⚠️ Scipy sigue con error: {e}")
...
print("=" * 70)
print("⏱️ Descargando modelos (solo la primera vez)...\n")

from paddleocr import PaddleOCR

# Configuración compatible con PaddleOCR 2.7.3
ocr_engine = PaddleOCR(
    use_angle_cls=True,  # Detectar orientación
    lang='es'            # Idioma español
)

print("\n✅ Motor OCR listo")
print("   🇪🇸 Idioma: Español")
print("   🎯 Versión 2.7.3 (estable)")
print("   ⚡ GPU activada automáticamente")
...
def mostrar_resultado_ocr(resultado_ocr, titulo="Texto Extraído"):
    """
...
def mejorar_imagen_para_ocr(imagen):
    ...
    # 2) Aumentar contraste (hace el texto más visible)
...
# ═══════════════════════════════════════════════════════════════════════
# DETECTORES DE TIPO DE DOCUMENTO (VERSIÓN MEJORADA)
# ═══════════════════════════════════════════════════════════════════════
...
def limpiar_texto_ocr(texto):
    """
    Limpia texto del OCR para mejorar detección
...
def detectar_tipo_documento(texto_ocr):
    """
    Detecta el tipo de documento automáticamente (VERSIÓN MEJORADA)

    Mejoras:
    - Búsqueda flexible (sin acentos, sin espacios exactos)
    - Scoring ponderado (múltiples palabras clave)
...
def mostrar_deteccion(deteccion):
    """
    Muestra el resultado de detección de forma visual
    """
    print(f"\n{'=' * 70}")
    print("🔍 DETECCIÓN DE DOCUMENTO")
    print(f"{'=' * 70}\n")
    ...
    if tipo == "DESCONOCIDO":
        print("❌ NO SE PUDO IDENTIFICAR EL DOCUMENTO")
        print("\n📊 Scores de detección (debug):")
    ...
    print(f"📊 SCORE: {deteccion['score']} puntos")
    print(f"🎨 COLOR: {deteccion.get('color', 'N/A')}")
    print(f"🌎 PAÍS: {deteccion.get('pais', 'N/A')}")

    # Mostrar scores de debug
    print(f"\n📊 Scores de todos los tipos:")
    for t, s in deteccion["debug_scores"].items():
        simbolo = "✅" if t == tipo else "  "
    ...
    if necesita_reverso:
        print(f"\n⚠️  REQUIERE FOTO DEL REVERSO")
        print("   (Para obtener dirección completa)")
    else:
        print(f"\n✅ NO REQUIERE REVERSO")
        print("   (Todos los datos están en una sola página)")

    print(f"\n{'=' * 70}")


print("✅ Detector de documentos mejorado (v2.0)")

# ═══════════════════════════════════════════════════════════════════════
# PARSER 1: DNI (VERSIÓN v4.4 - ESTADO CIVIL CORREGIDO)
# ═══════════════════════════════════════════════════════════════════════
...
def parsear_dni(texto_ocr, tipo_dni="DNI_AZUL"):
    ...
    if idx_prenombres >= 0:
        for i in range(idx_prenombres - 1, max(idx_prenombres - 4, -1), -1):
            linea = lineas[i]
            if re.match(r'^[A-ZÁÉÍÓÚÑ\s]{10,}$', linea):
    ...
    idx_emision = buscar_indice(r'Emisi[oó]n')
    ...
    idx_dir = buscar_indice(r'Direcci[oó]n')
...
print("✅ Parser DNI v4.4 (estado civil corregido)")

# ═══════════════════════════════════════════════════════════════════════
# PARSER REVERSO: DNI ELECTRÓNICO (con MRZ y dirección pegada)
# ═══════════════════════════════════════════════════════════════════════

def parsear_reverso_dni(texto_ocr, datos_frente=None):
    """
    Extrae campos del reverso del DNI
    - Dirección (puede estar pegada: AVENIDARICARDOPALMA323)
    - Ubigeo desde texto visible o desde MRZ si no está visible
    - Fechas desde MRZ si no están en texto normal
    """
    import re
    ...
    # ═══════════════════════════════════════════════════════
    # DIRECCIÓN (pegada o separada)
    # ═══════════════════════════════════════════════════════

    direccion_completa = []

    # Buscar líneas que contengan direcciones típicas
    patrones_direccion = [
        r'(AVENIDA|AVENI|AV\.?|CALLE|CA\.?|JIRON|JR\.?|PASAJE|PSJ\.?).*',
        r'.*\d{1,4}.*',  # Líneas con números (ej: 323)
        r'.*(DPTO|DEPTO|DEPT|PISO|INTERIOR|INT|MZ|LOTE|URB).*'
    ]
    ...
    for linea in lineas:
        for patron in patrones_direccion:
            if re.search(patron, linea, re.IGNORECASE):
                ...
                # Separar si está pegado (AVENIDARICARDO → AVENIDA RICARDO)
                if re.match(r'^(AVENIDA|CALLE|JIRON|PASAJE)[A-Z]', linea_limpia, re.IGNORECASE):
                    linea_limpia = re.sub(
                        r'(AVENIDA|CALLE|JIRON|PASAJE)([A-Z])',
                        r'\1 \2',
                        linea_limpia,
                        flags=re.IGNORECASE
                    )
                ...
                if linea_limpia not in direccion_completa:
                    direccion_completa.append(linea_limpia)
                break

    if direccion_completa:
        datos["direccion"] = ' '.join(direccion_completa).strip()
        # Limpiar espacios múltiples
        datos["direccion"] = re.sub(r'\s+', ' ', datos["direccion"])

    # ═══════════════════════════════════════════════════════
    # UBIGEO (Departamento/Provincia/Distrito)
    # ═══════════════════════════════════════════════════════

    # Estrategia 1: Buscar patrón LIMA/LIMA/MIRAFLORES o LIMA LIMA MIRAFLORES
    ubigeo_match = re.search(
        r'([A-ZÑÁÉÍÓÚ\s]+)[/\s]+([A-ZÑÁÉÍÓÚ\s]+)[/\s]+([A-ZÑÁÉÍÓÚ\s]+)',
        texto_ocr,
        re.IGNORECASE
    )

    if ubigeo_match:
        depto, prov, dist = ubigeo_match.groups()
        # Verificar que no sean números ni códigos
        if not re.search(r'\d', depto):
            datos["departamento"] = depto.strip()
        if not re.search(r'\d', prov):
            datos["provincia"] = prov.strip()
        if not re.search(r'\d', dist):
            datos["distrito"] = dist.strip()

    # Estrategia 2: Buscar 3 palabras largas seguidas (ej: LIMA LIMA MIRAFLORES)
    if "departamento" not in datos:
        # Buscar líneas con 3 palabras de 4+ letras
        for linea in lineas:
            palabras = re.findall(r'\b[A-ZÑÁÉÍÓÚ]{4,}\b', linea)
            if len(palabras) >= 3:
                # Verificar que no sean códigos o números
                if not any(re.search(r'\d|PER|DNI|CUI', p) for p in palabras[:3]):
                    datos["departamento"] = palabras[0]
                    datos["provincia"] = palabras[1]
                    datos["distrito"] = palabras[2]
                    break

    # ═══════════════════════════════════════════════════════
    # PARSEAR MRZ (Machine Readable Zone)
    # ═══════════════════════════════════════════════════════

    # Buscar líneas con formato MRZ (muchos < y mayúsculas)
    lineas_mrz = [l for l in lineas if l.count('<') > 5]

    if len(lineas_mrz) >= 3:
        # Línea 1: I<PER41326541<5<<<<<<<<<<<<<<<
        linea1 = lineas_mrz[0]

        # Línea 2: 8709281F2701016PER<<<<<<<<<<<4
        # Formato: AAMMDD + Sexo + AAMMDD + País + Check
        linea2 = lineas_mrz[1]

        # Extraer fecha de nacimiento (primeros 6 dígitos)
        fnac_mrz = re.match(r'^(\d{2})(\d{2})(\d{2})', linea2)
        if fnac_mrz and "fecha_nacimiento" not in datos:
            anio, mes, dia = fnac_mrz.groups()
            anio_completo = f"19{anio}" if int(anio) > 50 else f"20{anio}"
            datos["fecha_nacimiento"] = f"{dia}/{mes}/{anio_completo}"
            datos["fecha_nacimiento_iso"] = f"{anio_completo}-{mes}-{dia}"

        # Extraer fecha de caducidad (después del sexo, 6 dígitos)
        fcad_mrz = re.search(r'[MF](\d{2})(\d{2})(\d{2})', linea2)
        if fcad_mrz:
            anio, mes, dia = fcad_mrz.groups()
            anio_completo = f"20{anio}" if int(anio) < 50 else f"19{anio}"
            datos["fecha_caducidad"] = f"{dia}/{mes}/{anio_completo}"

        # Línea 3: RIVADENEIRA<<MARIA<MONICA<<<<<
        # (ya tenemos nombres del frente, no necesitamos parsear)

    return datos


print("✅ Parser de reverso DNI (dirección pegada + ubigeo + MRZ)")

# ═══════════════════════════════════════════════════════════════════════
# PARSER 2: CARNET DE EXTRANJERÍA
# ═══════════════════════════════════════════════════════════════════════

def parsear_carnet_extranjeria(texto_ocr):
    """
    Extrae campos del Carnet de Extranjería
    """
    import re
    datos = {}
    ...
    # ═══════════════════════════════════════════════════════
    ce_match = re.search(
        r'(?:N[°º]?|No\.?|CARNÉ DE EXTRANJERÍA)\s*(\d{9,10})',
        texto_ocr,
        re.IGNORECASE
    )
    if ce_match:
...
print("✅ Parser de Carnet de Extranjería listo")
...
def parsear_pasaporte(texto_ocr):
    ...
    Usa la zona MRZ (Machine Readable Zone) para máxima precisión
    """
    import re
    datos = {}

    # ═══════════════════════════════════════════════════════
    # MÉTODO 1: ZONA MRZ (más confiable)
    # ═══════════════════════════════════════════════════════

    # MRZ Línea 1: P<CODAPELLIDO<<NOMBRES<<<<<<<<<<<
    ...
    # MRZ Línea 2: NUMERO123CODYYMMDDSFYYMMDD<<<<<<<<
    ...
    # ═══════════════════════════════════════════════════════
    # MÉTODO 2: TEXTO VISIBLE (complementario)
    # ═══════════════════════════════════════════════════════

    # Número de Pasaporte (si no se extrajo de MRZ)
    if "numero_pasaporte" not in datos:
        num_match = re.search(
            r'(?:Passport No|N[°º]? Pasaporte)[:\.\s]*([A-Z0-9]{7,9})',
            texto_ocr,
            re.IGNORECASE
        )
        if num_match:
    ...
    emision_match = re.search(
        r'(?:Date of Issue|Fecha de Expedici[oó]n)[:\.\s]*(\d{2})\s*([A-Z]{3})\s*(\d{4})',
        texto_ocr,
        re.IGNORECASE
    )
    if emision_match:
...
print("✅ Parser de Pasaportes listo")
...
def procesar_documento(ruta_frente, ruta_reverso=None):
    ...
    Args:
        ruta_frente: Ruta de la imagen del frente
        ruta_reverso: Ruta de la imagen del reverso (opcional)
    ...
    try:
        # ═══════════════════════════════════════════════════════
        # PASO 1: CARGAR Y EVALUAR IMAGEN DEL FRENTE
        # ═══════════════════════════════════════════════════════

        print("\n📸 Cargando imagen del frente...")
        img_frente = cv2.imread(ruta_frente)

        if img_frente is None:
            raise Exception(f"No se pudo cargar la imagen: {ruta_frente}")

        altura, ancho = img_frente.shape[:2]
        print(f"   ✅ Imagen cargada: {ancho}x{altura} px")

        # Evaluar calidad
        print("\n🔍 Evaluando calidad de imagen...")
        calidad = evaluar_calidad_imagen(img_frente)
        ...
        if calidad['es_fotocopia']:
            print("   📄 Fotocopia detectada")

        # Mejorar imagen si es necesaria
        if calidad['calidad'] == 'MALA' or calidad['nitidez'] < 500:
            print("\n🔧 Mejorando calidad de imagen...")
            img_procesada = mejorar_imagen(img_frente)
            img_rgb = cv2.cvtColor(img_procesada, cv2.COLOR_GRAY2RGB)
            print("   ✅ Imagen mejorada")
        else:
            print("\n✅ Imagen con calidad suficiente, no requiere mejora")
            img_rgb = cv2.cvtColor(img_frente, cv2.COLOR_BGR2RGB)

        # ═══════════════════════════════════════════════════════
        # PASO 2: APLICAR OCR AL FRENTE
        # ═══════════════════════════════════════════════════════

        print("\n🤖 Aplicando OCR al frente del documento...")
        resultado_ocr = ocr_engine.ocr(img_rgb, cls=True)

        # Extraer texto
        texto_frente = ""
        for bloque in resultado_ocr[0]:
            texto_frente += bloque[1][0] + "\n"

        print(f"   ✅ OCR completado: {len(texto_frente)} caracteres extraídos")

        # ═══════════════════════════════════════════════════════
        # PASO 3: DETECTAR TIPO DE DOCUMENTO
        # ═══════════════════════════════════════════════════════
        ...
        if deteccion["tipo"] == "DESCONOCIDO":
            raise Exception("Tipo de documento no reconocido")

        resultado["tipo_documento"] = deteccion["tipo"]

        # ═══════════════════════════════════════════════════════
        # PASO 4: EXTRAER DATOS DEL FRENTE
        # ═══════════════════════════════════════════════════════
        ...
        if tipo in ["DNI_AZUL", "DNI_ELECTRONICO"]:
            ...
            datos = parsear_pasaporte(texto_frente, tipo)
        else:
            raise Exception(f"Parser no implementado para: {tipo}")

        print(f"   ✅ {len(datos)} campos extraídos del frente")

        # ═══════════════════════════════════════════════════════
        # PASO 5: PROCESAR REVERSO (si existe)
        # ═══════════════════════════════════════════════════════

        if ruta_reverso:
            print("\n📸 Procesando reverso del documento...")

            img_reverso = cv2.imread(ruta_reverso)

            if img_reverso is None:
                print("   ⚠️ No se pudo cargar el reverso, continuando sin él")
            else:
                img_reverso_rgb = cv2.cvtColor(img_reverso, cv2.COLOR_BGR2RGB)

                # Aplicar OCR al reverso
                resultado_reverso = ocr_engine.ocr(img_reverso_rgb, cls=True)

                texto_reverso = ""
                for bloque in resultado_reverso[0]:
                    texto_reverso += bloque[1][0] + "\n"

                # PARSEAR REVERSO SEGÚN TIPO
                if tipo in ["DNI_AZUL", "DNI_ELECTRONICO"]:
                    datos = parsear_reverso_dni(texto_reverso, datos)
                elif tipo == "CARNET_EXTRANJERIA":
                    # Aquí irá el parser de reverso de CE
                    pass

                print(f"   ✅ Datos del reverso extraídos: {len(datos)} campos totales")
        ...
        campos_requeridos = ["dni", "nombres", "apellido_paterno", "fecha_nacimiento"]
        campos_faltantes = [c for c in campos_requeridos if c not in datos]

        if campos_faltantes:
            print(f"   ⚠️ Campos faltantes: {', '.join(campos_faltantes)}")
            resultado["errores"].append(f"Campos faltantes: {', '.join(campos_faltantes)}")
        else:
            print("   ✅ Todos los campos principales extraídos correctamente")

        resultado["datos"] = datos
        resultado["exito"] = len(campos_faltantes) == 0
        ...
        if resultado["exito"]:
            ...
            print("⚠️ PROCESAMIENTO COMPLETADO CON ADVERTENCIAS")
        print("=" * 70)
...
print("✅ Función principal cargada")

# ═══════════════════════════════════════════════════════════════════════
# PARSERS ADICIONALES (CARNET DE EXTRANJERÍA Y PASAPORTE)
# ═══════════════════════════════════════════════════════════════════════

def parsear_carnet_extranjeria(texto_ocr):
    """
    Parser para Carnet de Extranjería (por implementar)
    """
    import re
    datos = {}

    # TODO: Implementar parser de CE
    print("   ⚠️ Parser de Carnet de Extranjería en desarrollo")
...
def parsear_pasaporte(texto_ocr, tipo="PASAPORTE_PERUANO"):
    ...
    print("   ⚠️ Parser de Pasaporte en desarrollo")

    return datos


print("✅ Parsers adicionales cargados (en desarrollo)")
...
def mostrar_resultados(resultado):
    ...
    if resultado["tipo_documento"]:
        ...
        if "pais" in datos:
            print(f"   País: {datos.get('pais', 'N/A')}")
    ...
    if any(k in datos for k in ["fecha_emision", "fecha_caducidad"]):
        ...
        if "fecha_emision" in datos:
            print(f"   Fecha de Emisión: {datos['fecha_emision']}")
...
def subir_imagenes():
    """
    Permite subir imágenes desde tu computadora a Colab
    ...
    """
    print("=" * 70)
    print("📤 SUBIR IMÁGENES")
    print("=" * 70)

    print("\n📸 Selecciona la foto del FRENTE del documento:")
    print("   (DNI/Pasaporte/Carnet de Extranjería)")
    ...
    print(f"✅ Frente subido: {filename_frente}")

    # Preguntar por el reverso
    print("\n¿Este documento necesita foto del reverso?")
    print("   • DNI (azul o electrónico) → SÍ")
    print("   • Carnet de Extranjería → SÍ")
    print("   • Pasaporte → NO")

    respuesta = input("\n¿Subir reverso? (s/n): ").strip().lower()

    imagen_reverso = None

    if respuesta == 's':
        print("\n📸 Selecciona la foto del REVERSO:")
        ...
        if uploaded_reverso:
            ...
            print(f"✅ Reverso subido: {filename_reverso}")
...
print("✅ Función de subida de archivos lista")
...
print("🔹 OPCIÓN 1: Subir imágenes manualmente")
print("-" * 70)
print("""
# Ejecuta esta celda:
...
print("\n🔹 OPCIÓN 2: Usar imágenes desde URL")
print("-" * 70)
print("""
# Descargar desde URL:
...
print("\n🔹 OPCIÓN 3: Usar imágenes desde Google Drive")
print("-" * 70)
print("""
...
print("\n🔹 OPCIÓN 4: Procesar múltiples documentos")
print("-" * 70)
print("""
...
for persona, imgs in documentos.items():
    print(f"\\n{'='*70}")
    print(f"📄 Procesando: {persona.upper()}")
    print(f"{'='*70}")
...
print("\n" + "=" * 70)
print("💡 EJECUTA LA SIGUIENTE CELDA PARA EMPEZAR")
print("=" * 70)
...
print(f"✅ Frente subido: {ruta_frente}")

# Preguntar si necesita reverso
print("\n¿Este documento necesita foto del reverso?")
print("   • DNI (azul o electrónico) → SÍ")
print("   • Carnet de Extranjería → SÍ")
print("   • Pasaporte → NO")

necesita_reverso = input("\n¿Subir reverso? (s/n): ").lower().strip()
...
if necesita_reverso == 's':
    ...
    print(f"✅ Reverso subido: {ruta_reverso}")
...
if resultado["exito"]:
    print("\n" + "=" * 70)
    print("✅ ¿QUÉ QUIERES HACER CON ESTOS DATOS?")
    print("=" * 70)

    print("\n1️⃣  Exportar como JSON")
    print("2️⃣  Exportar como CSV")
    print("3️⃣  Ver datos crudos (para debugging)")
    print("4️⃣  Guardar en variable para usar después")

    # Mostrar JSON
    print("\n📄 DATOS EN FORMATO JSON:")
    print("-" * 70)
    datos_json = json.dumps(resultado["datos"], indent=2, ensure_ascii=False)
    print(datos_json)

    # Guardar en variable global
    datos_extraidos = resultado["datos"]
    print("\n✅ Datos guardados en la variable: datos_extraidos")
...
print("📤 Sube la imagen del FRENTE:")
...
print("=" * 70)
print("📄 TEXTO RAW DEL OCR (línea por línea)")
print("=" * 70)
print(texto_raw)
print("=" * 70)
...
print("\n✅ Texto guardado en 'texto_ocr.txt'")
print("\n📋 Copia este texto y envíamelo para análisis")

# DEBUG REVERSO
from google.colab import files

print("📤 Sube la imagen del REVERSO (DNI02-523X250.jpg):")
uploaded = files.upload()
imagen_path = list(uploaded.keys())[0]
...
print("=" * 70)
print("📄 TEXTO RAW DEL REVERSO")
print("=" * 70)
print(texto_raw)
print("=" * 70)

# ═══════════════════════════════════════════════════════════════════════
# CÓDIGO PARA INTEGRAR EN TU APP STREAMLIT
# ═══════════════════════════════════════════════════════════════════════

codigo_streamlit = '''
# ═══════════════════════════════════════════════════════════════════════
# INTEGRACIÓN EN app.py (tu sistema actual)
# ═══════════════════════════════════════════════════════════════════════
...
st.subheader("👨 Datos del Padre")

# Pestañas para elegir método
tab_ocr, tab_manual = st.tabs(["📸 Escanear DNI", "⌨️ Ingresar manualmente"])
...
with tab_ocr:
    ...
    if st.button("🔍 Extraer datos automáticamente", key="btn_padre"):
        if padre_frente:
            with st.spinner("Procesando documento..."):
                # AQUÍ LLAMAS A TU FUNCIÓN procesar_documento()
                resultado = procesar_documento(
                ...
                if resultado["exito"]:
                    st.success(f"✅ Documento procesado: {resultado['tipo']}")
                    ...
                    st.info("💡 Intenta con mejor calidad de imagen o usa ingreso manual")
        else:
            st.warning("⚠️ Debes subir al menos la foto del frente")
...
print("=" * 70)
print("💻 CÓDIGO PARA INTEGRAR EN TU STREAMLIT")
print("=" * 70)
...
print("\n✅ Código guardado en: integracion_streamlit.py")
print("   Puedes descargarlo y copiarlo a tu app.py")

# ═══════════════════════════════════════════════════════════════════════
# ANALIZAR RENDIMIENTO Y PRECISIÓN
# ═══════════════════════════════════════════════════════════════════════

def generar_reporte(resultados_lista):
    """
    Genera reporte de múltiples procesamientos
    ...
    if confianzas:
        ...
        print(f"\n🎯 CONFIANZA PROMEDIO: {promedio*100:.1f}%")
...
print("✅ Función de reportes lista")